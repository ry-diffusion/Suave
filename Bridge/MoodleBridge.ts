import {
  ...
  Module,
} from "@/app/(api)/api/moodle/GetAvailableModules/route";
import {
  Entry,
  QueryModule,
} from "@/app/(api)/api/moodle/GetCourseCompletionStatus/route";
...
export class MoodleBridge {
  ...
  async GetAvailableModules(
    courses: Course[]
  ): Promise<GetAvailableModulesResponse> {
  ...
  async GetCourseCompletionStatus(
    query: { courseId: number; modules: (ApiModule | Module)[] }[]
  ): Promise<Entry[]> {
    // Reduce the size of the query by removing unnecessary fields
    const optimizedQuery: { courseId: number; modules: QueryModule[] }[] =
      query.map(({ courseId, modules }) => {
        return {
          courseId,
