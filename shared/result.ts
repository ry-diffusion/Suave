...
export class Result<T> {
	...
	public ensure(
		...
		if (!predicate(this.value!)) {
			return Result.fail<T>(
				error instanceof Error ? error : error(this.value!),
			);
		}
	...
	public handle<E extends Error>(
		errorType: new (...args: any[]) => E,
		callback: (error: E) => Result<T>,
	): Result<T> {
	...
	public async handleAsync<E extends Error>(
		errorType: new (...args: any[]) => E,
		callback: (error: E) => Promise<Result<T>>,
	): Promise<Result<T>> {
		if (this.isFailure && this.error instanceof errorType) {
...
export class PromiseResult<T> {
	...
	static all<T>(promises: PromiseResult<T>[]): PromiseResult<T[]> {
		return new PromiseResult(() =>
			Promise.all(promises.map((p) => p.task())).then((results) => {
				if (results.some((result) => result.isFailure)) {
					return Result.fail<T[]>(
						results.find((result) => result.isFailure)!.error!,
					);
				}
