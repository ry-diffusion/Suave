	...
	implements IProvider<IFGoianoPresencialCredentials, AuthContext>
{
	...
	refreshAuth(authContext: AuthContext): PromiseResult<AuthContext> {
		const doRefresh = async () => {
			...
			if (authContext.api) {
				const refreshResult = await this.apiProvider
					.refreshToken(authContext.api)
					.run();
				if (refreshResult.isSuccess) {
					...
					if (authContext.ead) {
						...
						const reauthResult = await this.apiProvider
							.authenticate(creds)
							.run();
			...
			if (authContext.ead) {
				const siteInfoResult = await this.eadProvider
					.getSiteInfo(authContext.ead)
					.run();
	...
	async getIdentity(): Promise<IIdentity> {
		...
		return await this.eadProvider
			.getSiteInfo(this.authContext.ead)
			.ensure(
